...
from models.conditional_unet import ConditionalUNet, ConditionalUNet_Celeba

class ConditionalDDPM_Model_CIFAR(DDPM_Model):
...
class ConditionalDDPM_Model_Celeba(DDPM_Model):
    """类别条件 DDPM，实现与 DDPM_Model 基本一致，但在 UNet 前向中加入标签。"""

    def __init__(self, dataloader, num_classes: int = 10, **kwargs):
        super().__init__(dataloader, **kwargs)
        # 用条件 UNet 替换
        self.unet = ConditionalUNet_Celeba(num_classes=num_classes).to(self.device)
        self.num_classes = num_classes
